...
import { GET_my_list_application_card } from "@/connection/credentials/application";
...
export default function Page({ nip }: { nip: string }) {
  // Use SWR with the function fetcher
  const { data, error, isLoading } = useSWR<ApplicationCard[]>(
    nip ? `GET_my_list_application_card-${nip}` : null,
    async () => {
      try {
        const result = await GET_my_list_application_card(nip);
  ...
  const handleCardClick = async () => {
    // none because by default all accessAs is 5
  };
  // --- State Manajement---
  ...
  if (data && data.length === 0) {
    return (
      <div className="w-screen max-w-full mx-auto p-4 text-center text-gray-500 bg-gray-50 rounded-lg">
        Anda tidak memiliki Aplikasi PK yang dalam proses pengajuan.
      </div>
    );
  }
