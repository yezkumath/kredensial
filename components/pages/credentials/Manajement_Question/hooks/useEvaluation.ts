  ...
  const getAnswer = async () => {
    try {
      ...
      switch (function_menu) {
        ...
        case FUNCTION_MENU.KOMITE_VIEW_EVAL: // Case 4
          console.log("🔍 Filtering for target_nip:", target_nip);

          filteredAnswers = database.filter((answer: any) => {
            // Convert both to strings for comparison
            const answerTargetNip = String(answer.target_nip);
            const compareNip = String(target_nip);
            const matches = answerTargetNip === compareNip;

            if (!matches) {
              console.log("❌ Mismatch:", answerTargetNip, "!==", compareNip);
            }
            return matches;
          });
