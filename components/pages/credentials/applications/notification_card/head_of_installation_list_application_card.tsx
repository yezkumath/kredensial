...
import { GET_head_of_installation_list_application_card } from "@/connection/credentials/application";
...
export default function Page({ nip }: { nip: string }) {
  ...
  const { data, error, isLoading } = useSWR<ApplicationCard[]>(
    nip ? `GET_head_of_installation_list_application_card-${nip}` : null,
    async () => {
      try {
        const result = await GET_head_of_installation_list_application_card(
          nip
        );
